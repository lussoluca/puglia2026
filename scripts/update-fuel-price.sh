#!/usr/bin/env bash
# Update the GPL fuel price from MIMIT open data CSV
...
# Litres budgeted for the whole trip: ~2.800 km at ~11 L/100km
TRIP_LITRES=308
...
GPL_TOTAL=$(awk "BEGIN {printf \"%.0f\", $TRIP_LITRES * $GPL_PRICE}")
echo "Estimated GPL cost: ~${GPL_TOTAL} EUR (${TRIP_LITRES}L x ${GPL_PRICE} EUR/L)"
...
// Recalculate grand total; categories without a numeric total are skipped
...
console.log('Updated budget.json: gpl=' + '${GPL_PRICE}' + ' EUR/L, total=~' + rounded + ' EUR');
"
